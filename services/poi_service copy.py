...
load_dotenv()
from utils.data_processing import process_poi_for_clean_table, generate_llm_description
...
class PoiService:
    ...
    async def add_new_poi(self, poi_ids: List[UUID], max_total_reviews: int = 10000) -> Dict[str, Any]:
        """
        Lấy thông tin POI từ bảng Poi theo danh sách IDs,
        sau đó clean, normalize, generate LLM description và insert/update vào bảng PoiClean.
        
        Quy trình:
        1. Lấy data từ bảng Poi theo id
        2. Clean data (xử lý null values, opening_hours)
        3. Normalize data (tính normalize_stars_reviews)
        4. Generate LLM description (poi_type_new, main_subcategory, specialization, suitability)
        5. Insert/Update vào bảng PoiClean
        
        Args:
            poi_ids: List UUID của các POI cần xử lý
            max_total_reviews: Giá trị max total_reviews để normalize (lấy từ DB)
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                # Step 2, 3, 4: Process each POI (Extract -> Clean -> Normalize -> LLM)
                for row in rows:
                    try:
                        poi_data = dict(row)
                        
                        # Process data through pipeline (Extract -> Clean -> Normalize)
                        ...
                        # Step 4: Generate LLM description
                        llm_result = await generate_llm_description(
                            processed_data, 
                            client=self.openai_client
                        )
                        
                        # Lưu kết quả LLM vào trường "llm"
                        processed_data["llm"] = llm_result 
                        
                        # Step 5: Insert/Update vào bảng PoiClean
    ...
    async def _upsert_poi_clean(self, conn: asyncpg.Connection, data: Dict[str, Any]):
        ...
        Args:
            ...
            data: Processed POI data (including llm field)
        """
        # Convert opening_hours to JSON string
        opening_hours_json = json.dumps(data.get("opening_hours", []))
        
        # Convert llm result to JSON string
        llm_json = json.dumps(data.get("llm")) if data.get("llm") else None
