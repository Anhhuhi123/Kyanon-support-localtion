...
#### ✅ Quy Tắc 1: Food & Local Flavours → KHÔNG cần chèn restaurant vào Meal-Time 

Khi user chọn **Food & Local Flavours**, pool đã có sẵn `Cafe & Bakery` và `Restaurant`.
User có thể ghé ăn bất kỳ lúc nào theo alternation tự nhiên → **Không cần chèn restaurant thêm meal-time**.

#### ✅ Quy Tắc 2: Không có Food & Local Flavours → CÓ chèn chèn restaurant vào Meal-Time 
...
> Ví dụ arrive 12h10 thì sẽ chèn restaurants thay vì cafe cho dù lúc này cafe_counter = 2
