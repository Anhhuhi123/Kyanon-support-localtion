...
qdrant_vector_store = None  # Will be set from server.py
...
#         result_qdrant = await qdrant_vector_store.ingest_all_poi()
...
async def new_sync_pois(payload: PoiRequest) -> dict:
    ...
    add_ids = [str(i) for i in (payload.sync_ids or [])]
    delete_ids = [str(i) for i in (payload.delete_ids or [])]
    try: 
        # xử lí trùng lặp id giữa add và update và delete
        add_set = set(add_ids)
        delete_set = set(delete_ids)
